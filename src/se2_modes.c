...
static void se2_select_mode(igraph_integer_t const time, se2_tracker *tracker)
{
  ...
  if ((tracker->allowed_to_merge) &&
      (tracker->time_since_last[SE2_MERGE] > 1) &&
                       ...
                       se2_partition *partition, se2_tracker *tracker, igraph_integer_t const time)
{
  ...
  se2_partition_shuffle(partition);
  se2_select_mode(time, tracker);

  switch (tracker->mode) {
  case SE2_TYPICAL:
    se2_typical_mode(graph, weights, partition, tracker);
    break;
  case SE2_BUBBLE:
  ...
  case SE2_MERGE:
    ...
    break;
  case SE2_NURTURE:
    se2_nurture_mode(graph, partition, tracker);
    break;
  case SE2_NUM_MODES:
    // Never occurs.
    break;
